...
/**
 * Class Livemesh_Table_Rate_Shipping_Method.
 *
 *  Livemesh Table Rate Shipping method for WooCommerce
 *
 */
class Livemesh_Table_Rate_Shipping_Method extends WC_Shipping_Method {

    public function __construct($instance_id = 0) {

        parent::__construct($instance_id);

        $this->id = 'livemesh_table_rate_shipping';
        $this->method_title = __('Livemesh Table Rate Shipping', 'livemesh-tr-shipping');
        $this->method_description = __('Flexible and conditional shipping rate', 'livemesh-tr-shipping');
        $this->supports = array('shipping-zones', 'instance-settings');

        $this->init();

        add_filter('woocommerce_shipping_' . 'livemesh_table_rate_shipping' . '_instance_settings_values', array($this, 'save_settings'), 10, 2);

        do_action('woocommerce_livemesh_table_rate_shipping_method_init', $this);
    }

    /**
     *
     * Initialize the fields and settings part of Livemesh Table Rate Shipping method.
     *
     */
    function init() {
    ...
    public function get_instance_settings() {
        ...
        return array(
            'title' => array(
                'title' => __('Title', 'livemesh-tr-shipping'),
                'type' => 'title_text',
                'placeholder' => __('Shipping title', 'livemesh-tr-shipping'),
            ),
            'livemesh_table_rate_shipping_settings' => array(
                'type' => 'livemesh_table_rate_shipping_settings',
            ...
            livemesh_table_rate_shipping_settings above, but the below will help us utilize the get/set functionality of $instance_settings
            */
    ...
    public function validate_none_field($key, $value) {
        ...
        $value = wc_clean($_POST['_ltrs_shipping_method'][$key]);
    ...
    /**
     * Load the instance settings and render the 'livemesh_table_rate_shipping_settings' field type.
     *
     */
    public function generate_livemesh_table_rate_shipping_settings_html($key, $data) {
        ...
        <div class="ltrs-shipping-method-wrap" style="margin-right: 300px;" id="poststuff">
            <div id="ltrs_conditions" class="postbox ">
                <h2 class="" style="border-bottom: 1px solid #eee;">
                    <span><?php _e('Conditions', 'livemesh-tr-shipping'); ?></span></h2>
                <div class="inside"><?php
                    require_once LTRS_PLUGIN_DIR . '/includes/admin/views/shipping-conditions.php';
                    ?></div>
            </div>

            <div id="ltrs_settings" class="postbox ">
                <style>.ltrs-shipping-title { display: none; }</style>
                <h2 class="" style="border-bottom: 1px solid #eee;">
                    <span><?php _e('Settings', 'livemesh-tr-shipping'); ?></span></h2>
                <div class="inside"><?php
                    require_once LTRS_PLUGIN_DIR . '/includes/admin/views/shipping-settings.php';
                    ?></div>
            </div>
        </div>
    ...
    public function generate_title_text_html($key, $data) {
        ...
        ob_start();
        ?></table>
        <div class="ltrs-title-text-wrap">
            <legend class="screen-reader-text"><span><?php echo wp_kses_post($data['title']); ?></span></legend>
            <input class="input-text regular-input ltrs-title-text <?php echo esc_attr($data['class']); ?>"
    ...
    public function save_settings($settings, $class) {
        ...
        foreach (LTRS()->table_rates_helper->get_table_rates_options() as $key => $option) {
    ...
    protected function calculate_shipping_cost($package, $rate_id, $args) {
        ...
        /** @var LTRS_Table_Rate_Abstract $option */
        foreach (LTRS()->table_rates_helper->get_table_rates_options() as $option) {
        ...
        return apply_filters('ltrs_calculate_shipping_costs', $cost, $package, $rate_id, $this);
    ...
    public function calculate_shipping($package = array()) {
        ...
        if (lwc_match_conditions($conditions, array('context' => 'ltrs', 'package' => $package))) {
            ...
            $rate_args = apply_filters('ltrs_shipping_rate', array(
                'id' => $this->get_rate_id(),
