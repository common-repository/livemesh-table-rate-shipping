jQuery( function ( $ ) {
    ...
    function rateTypeChanged( rateTypeSelector, resetValues ) {
        // Loop through each rate type select field
        rateTypeSelector.each( function () {
            var optionSelected = $(this).find( "option:selected" );
            var valueSelected = optionSelected.val();
            var $parent = $(this).closest( '.ltrs-input-group-list' );
            $parent.find( '[data-rate-type]' ).hide();
            $parent.find( '[data-rate-type=' + valueSelected + ']' ).show();
            if (resetValues) {
    ...
    $( '.ltrs-table-rates-settings' ).on( 'click', '.delete-repeater-row', function () {
        $( this ).parents( '.repeater-row' ).slideUp( function () {
            $( this ).remove();
        } );
    } );

    $('.ltrs-table-rates-settings .repeater-wrap').on('change', 'select.ltrs-select-rate-type', function() {
        rateTypeChanged( $( this ), true );
    } );

    rateTypeChanged( $( 'select.ltrs-select-rate-type' ), false );

    /*-------  Price field input ---------- */
    $( document.body ).on( 'change keyup', '.ltrs_input_decimal[type=text]', function () {
    ...
    $( document.body ).on( 'blur', '.ltrs_input_price[type=text]', function () {
        $( '.wc_error_tip' ).fadeOut( '100', function () {
            $( this ).remove();
        } );
    } )
        .on( 'keyup change', '.ltrs_input_price[type=text]', function () {
            var value = $( this ).val();
